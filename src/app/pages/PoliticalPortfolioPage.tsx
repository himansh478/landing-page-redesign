...
import { politicalHighlights } from './politicalHighlights.ts';
import { PortfolioVideo } from './commercialHighlights.ts';

export function PoliticalPortfolioPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <Link 
            ...
            className="inline-flex items-center gap-2 text-slate-500 hover:text-red-600 font-bold transition-colors group"
          >
      ...
      {/* Hero Section */}
      <section className="py-20 bg-[radial-gradient(circle_at_top_right,rgba(220,38,38,0.05)_0%,transparent_50%)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h1 className="text-6xl md:text-8xl font-black text-slate-900 mb-8 tracking-tight">
              Political <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-orange-600">Portfolio</span>
            </h1>
            <p className="text-xl md:text-2xl text-slate-500 max-w-3xl mx-auto font-light leading-relaxed">
              Powerful campaign messaging and professional political event coverage.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-12">
            <h2 className="text-4xl font-black text-slate-900 tracking-tight">Campaign <span className="text-red-600">Highlights</span></h2>
            <div className="h-1 flex-grow mx-8 bg-slate-100 rounded-full" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
            {politicalHighlights.map((video, index) => (
              <motion.div
                key={video.id}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="group relative bg-white rounded-3xl overflow-hidden border border-slate-100 hover:border-red-200 transition-all cursor-pointer shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-red-500/10"
              >
                {/* Thumbnail */}
                <div className="relative w-full aspect-[9/16] bg-slate-100 p-1">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 transform scale-50 group-hover:scale-100">
                    <div className="w-16 h-16 bg-white text-red-600 rounded-full flex items-center justify-center shadow-2xl">
                      <Play className="w-7 h-7 fill-red-600 text-red-600" />
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-white border-t border-slate-50">
                  <h3 className="text-sm font-black text-slate-900 leading-tight group-hover:text-red-600 transition-colors line-clamp-2 uppercase tracking-tight">
                    {video.title}
                  </h3>
                </div>
              </motion.div>
      ...
      <section className="py-32 bg-slate-900 relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(220,38,38,0.2)_0%,transparent_70%)] pointer-events-none" />
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          >
            <h2 className="text-5xl md:text-7xl font-black text-white mb-8 tracking-tight leading-[1.1]">
              Make an <span className="text-red-400">Impact</span>
            </h2>
            ...
            <div className="flex gap-6 flex-wrap justify-center">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-red-600 text-white px-10 py-5 rounded-2xl text-xl font-black shadow-2xl shadow-red-500/40 transition-all hover:bg-red-500 active:scale-95 uppercase tracking-widest text-sm"
              >
                Get Started
              </motion.button>
            </div>
