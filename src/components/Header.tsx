...
export function Header() {
  ...
  const handleShare = async () => {
    const url = typeof window !== 'undefined' ? window.location.origin : 'https://www.cwaya.me';

    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          text: 'Check out Cwaya for premium video editing, photography, and social media management!',
          url,
        });
      } catch {
        // user cancelled
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          >
            ...
            <span className="text-slate-900 text-lg sm:text-xl font-black text-gradient" style={{ fontFamily: 'Outfit, Inter, sans-serif' }}>
              Cwaya
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center justify-end gap-3 lg:gap-4">
            ...
            <Link
              href="/raw-clips"
              className="flex items-center gap-1.5 text-indigo-600 hover:text-purple-600 px-3 py-2 font-bold transition-colors text-sm"
            >
              🎬 Raw Clips
            </Link>
