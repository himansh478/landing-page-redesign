...
import { carDeliveryReels } from './carDeliveryReels.ts';
import { bikeDeliveryReels } from './bikeDeliveryReels.ts';
...
export interface ReelVideo {
  ...
  thumbnail: string;
  videoUrl: string;
  type: 'car' | 'bike' | 'engagement';
}

export function ReelPortfolioPage() {
  const [selectedVideo, setSelectedVideo] = useState<ReelVideo | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isBookingModalOpen, setIsBookingModalOpen] = useState(false);
  const [isSamplesModalOpen, setIsSamplesModalOpen] = useState(false);

  const handleVideoClick = (video: ReelVideo) => {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h1 className="text-6xl md:text-8xl font-black text-slate-900 mb-8 tracking-tight">
              Reel Edit <span className="text-gradient">Portfolio</span>
            </h1>
            <p className="text-xl md:text-2xl text-slate-500 max-w-3xl mx-auto font-light leading-relaxed">
              Viral Instagram reels with trending effects, beat-synced editing, and maximum engagement potential.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-8 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
            ...
            {['Car Delivery', 'Bike Delivery'].map((cat) => (
               <button key={cat} className="px-8 py-3 bg-white border border-slate-200 text-slate-600 rounded-2xl font-black uppercase tracking-widest text-xs hover:border-indigo-200 hover:bg-slate-50 transition-all">
                 {cat}
               </button>
            ))}
          </div>
      ...
      {/* Car Delivery Edit Section - 9:16 Aspect Ratio */}
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-12">
            <h2 className="text-4xl font-black text-slate-900 tracking-tight">Car <span className="text-pink-600">Delivery</span> Edit</h2>
            <div className="h-1 flex-grow mx-8 bg-slate-100 rounded-full" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
            {carDeliveryReels.map((video, index) => (
              <motion.div
                key={video.id}
                initial={{ opacity: 0, y: 20 }}
      ...
      {/* Bike Delivery Section - 9:16 Aspect Ratio */}
      <section className="py-20 bg-slate-50/50 border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-12">
            <h2 className="text-4xl font-black text-slate-900 tracking-tight">Bike <span className="text-indigo-600">Delivery</span></h2>
            <div className="h-1 flex-grow mx-8 bg-slate-200 rounded-full" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
            {bikeDeliveryReels.map((video, index) => (
              <motion.div
                key={video.id}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          >
            <h2 className="text-5xl md:text-7xl font-black text-white mb-8 tracking-tight">
              Ready to Go <span className="text-indigo-400">Viral?</span>
            </h2>
            <p className="text-xl text-slate-400 mb-12 font-light leading-relaxed">
              Get your Instagram reels edited with trending effects and maximum engagement potential.
            </p>
            <div className="flex gap-6 flex-wrap justify-center">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-indigo-600 text-white px-10 py-5 rounded-2xl text-xl font-black shadow-2xl shadow-indigo-500/40 transition-all hover:bg-indigo-500 active:scale-95 uppercase tracking-widest text-sm"
              >
                Get Started
              </motion.button>
            </div>
