...
export async function registerPartner(input: RegisterPartnerInput) {
  ...
  // 3. Turnstile CAPTCHA Token Verify
  if (!input.turnstileToken) {
    return { success: false, error: 'Security verification check missing hai!' };
  }

  const isCaptchaValid = await verifyTurnstileToken(input.turnstileToken);
  if (!isCaptchaValid) {
    return { success: false, error: 'Security check fail ho gaya. Kripya captcha phir se verify karein.' };
  }
