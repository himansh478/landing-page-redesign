...
import { weddingHighlights } from './weddingHighlights.ts';

export interface WeddingReelVideo {
  id: number;
  title: string;
  thumbnail: string;
  videoUrl: string;
...
export function WeddingPortfolioPage() {
  const [selectedVideo, setSelectedVideo] = useState<WeddingReelVideo | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isBookingModalOpen, setIsBookingModalOpen] = useState(false);
  const [isSamplesModalOpen, setIsSamplesModalOpen] = useState(false);

  const handleVideoClick = (video: WeddingReelVideo) => {
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <Link 
            ...
            className="inline-flex items-center gap-2 text-slate-500 hover:text-rose-600 font-bold transition-colors group"
          >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h1 className="text-6xl md:text-8xl font-black text-slate-900 mb-8 tracking-tight">
              Wedding <span className="text-gradient">Portfolio</span>
            </h1>
            <p className="text-xl md:text-2xl text-slate-500 max-w-3xl mx-auto font-light leading-relaxed">
              Timeless wedding films and highlights capturing your precious moments in vertical cinematic glory.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-8 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
            ...
            {['Highlights', 'Ceremony', 'Storytelling', 'Reception', 'Celebration'].map((cat) => (
               <button key={cat} className="px-8 py-3 bg-white border border-slate-200 text-slate-600 rounded-2xl font-black uppercase tracking-widest text-xs hover:border-rose-200 hover:bg-slate-50 transition-all whitespace-nowrap">
                 {cat}
               </button>
            ))}
          </div>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-12">
            <h2 className="text-4xl font-black text-slate-900 tracking-tight">Quick <span className="text-pink-600">Highlights</span></h2>
            <div className="h-1 flex-grow mx-8 bg-slate-100 rounded-full" />
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
            {weddingHighlights.map((video, index) => (
              <motion.div
                key={video.id}
                initial={{ opacity: 0, y: 20 }}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          >
            <h2 className="text-5xl md:text-7xl font-black text-white mb-8 tracking-tight leading-[1.1]">
              Preserve Your <span className="text-rose-400">Love Story</span>
            </h2>
            <p className="text-xl text-slate-400 mb-12 font-light leading-relaxed">
              Let us create a cinematic wedding film that you and your family will treasure for a lifetime.
            </p>
            <div className="flex gap-6 flex-wrap justify-center">
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-rose-600 text-white px-10 py-5 rounded-2xl text-xl font-black shadow-2xl shadow-rose-500/40 transition-all hover:bg-rose-500 active:scale-95 uppercase tracking-widest text-sm"
              >
                Get Started
              </motion.button>
            </div>
