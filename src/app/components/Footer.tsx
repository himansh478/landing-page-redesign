import { Camera, Mail, Phone, MapPin, Instagram, Youtube, Send } from 'lucide-react';

export function Footer() {
  return (
    <footer className="bg-slate-50 border-t border-slate-200 relative overflow-hidden">
      {/* Background Glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-32 bg-indigo-500/5 blur-3xl rounded-full" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-12 mb-8 sm:mb-12">
          {/* Brand */}
          <div>
            <div className="flex items-center gap-2 sm:gap-3 mb-4 font-sans">
              <Camera className="w-6 sm:w-8 h-6 sm:h-8 text-indigo-600 flex-shrink-0" />
              <span className="text-slate-900 text-xl sm:text-3xl font-black tracking-tight">Cwaya</span>
            </div>
            <p className="text-slate-500 mb-6 text-sm sm:text-base font-light">
              Your complete creative and tech powerhouse. Premium results, unbeatable prices.
            </p>
            <div className="flex gap-3">
              <a
                href="https://www.instagram.com/creative_shiva_01?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw=="
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-white border border-slate-200 hover:bg-gradient-to-tr hover:from-pink-500 hover:to-orange-500 rounded-lg flex items-center justify-center text-slate-500 hover:text-white transition-all hover:scale-110 shadow-sm"
                aria-label="Instagram"
              >
                <Instagram className="w-5 h-5" />
              </a>
              <a
                href="https://chat.whatsapp.com/Ftagt0XUzkQI0zSfHZMXxI"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-white border border-slate-200 hover:bg-green-500 rounded-lg flex items-center justify-center text-slate-500 hover:text-white transition-all hover:scale-110 shadow-sm"
                aria-label="WhatsApp"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>
              </a>
              <a
                href="https://youtube.com/@creativeshivavlogs?si=sIwTtBGWIiB3k7q1"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-white border border-slate-200 hover:bg-red-600 rounded-lg flex items-center justify-center text-slate-500 hover:text-white transition-all hover:scale-110 shadow-sm"
                aria-label="YouTube"
              >
                <Youtube className="w-5 h-5" />
              </a>
              <a
                href="https://t.me/creativeshiva01"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-white border border-slate-200 hover:bg-blue-500 rounded-lg flex items-center justify-center text-slate-500 hover:text-white transition-all hover:scale-110 shadow-sm"
                aria-label="Telegram"
              >
                <Send className="w-5 h-5" />
              </a>
            </div>
          </div>

          {/* Services */}
          <div>
            <h3 className="text-slate-900 font-bold mb-4 text-sm sm:text-base tracking-wide uppercase">Services</h3>
            <ul className="space-y-2 sm:space-y-3">
              <li>
                <a href="#" className="text-slate-500 hover:text-indigo-600 transition-colors text-sm sm:text-base">
                  Video Editing
                </a>
              </li>
              <li>
                <a href="#" className="text-slate-500 hover:text-indigo-600 transition-colors text-sm sm:text-base">
                  Professional Shoots
                </a>
              </li>
              <li>
                <a href="#" className="text-slate-500 hover:text-indigo-600 transition-colors text-sm sm:text-base">
                  Social Media Management
                </a>
              </li>
              <li>
                <a href="#" className="text-slate-500 hover:text-indigo-600 transition-colors text-sm sm:text-base">
                  Web Development / AI bots
                </a>
              </li>
              <li>
                <a href="#" className="text-slate-500 hover:text-indigo-600 transition-colors">
                  AI services
                </a>
              </li>
            </ul>
          </div>

          {/* Company */}
          <div>
            <h3 className="text-slate-900 font-bold mb-4 text-sm sm:text-base tracking-wide uppercase">Company</h3>
            <ul className="space-y-2 sm:space-y-3">
              <li>
                <a href="#" className="text-slate-500 hover:text-indigo-600 transition-colors">
                  "Budget-Friendly Solutions, Professional Standards"
                </a>
              </li>
              <li>
                <a href="#portfolio" className="text-slate-500 hover:text-indigo-600 transition-colors">
                  Portfolio
                </a>
              </li>
              <li>
                <a href="#pricing" className="text-slate-500 hover:text-indigo-600 transition-colors">
                  Pricing
                </a>
              </li>
              <li>
                <a href="#membership" className="text-slate-500 hover:text-indigo-600 transition-colors">
                  Membership
                </a>
              </li>
              <li>
                <a href="/blog" className="text-slate-500 hover:text-indigo-600 transition-colors">
                  Blog
                </a>
              </li>
              <li>
                <a href="/about-us" className="text-slate-500 hover:text-indigo-600 transition-colors">
                  About Us
                </a>
              </li>
              <li>
                <a href="/contact-us" className="text-slate-500 hover:text-indigo-600 transition-colors">
                  Contact
                </a>
              </li>
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="text-slate-900 font-bold mb-4 tracking-wide uppercase">Get in Touch</h3>
            <ul className="space-y-3">
              <li className="flex items-start gap-3 text-slate-500">
                <Mail className="w-5 h-5 mt-0.5 flex-shrink-0 text-indigo-500" />
                <span>Sudhanshugour89@gmail.com </span>
              </li>
              <li className="flex items-start gap-3 text-slate-500">
                <Phone className="w-5 h-5 mt-0.5 flex-shrink-0 text-indigo-500" />
                <span>+91 81203 17074</span>
              </li>
              <li className="flex items-start gap-3 text-slate-500">
                <MapPin className="w-5 h-5 mt-0.5 flex-shrink-0 text-indigo-500" />
                <span>Indore–Bhopal Road, Near Bus Stand Sehore – 466001, MP, India</span>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="pt-6 sm:pt-8 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4 text-center sm:text-left relative z-10">
          <p className="text-slate-400 text-xs sm:text-sm">
            ©All rights reserved.
          </p>
          <div className="flex gap-4 sm:gap-6 text-xs sm:text-sm flex-wrap justify-center">
            <a href="/privacy-policy" className="text-slate-400 hover:text-indigo-600 transition-colors">
              Privacy Policy
            </a>
            <a href="/terms-conditions" className="text-slate-400 hover:text-indigo-600 transition-colors">
              Terms & Conditions
            </a>
            <a href="#" className="text-slate-400 hover:text-indigo-600 transition-colors">
              Cookie Policy
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
}
