...
import { Heart, Smartphone, Megaphone, Briefcase, Zap, Cross, TrendingUp, Film, ArrowLeft, Check, Eye } from 'lucide-react';
import { Link, useNavigate } from 'react-router';
import { useState } from 'react';
import { ServiceBookingForm } from '../components/ServiceBookingForm';
import { ViewSamplesModal } from '../components/ViewSamplesModal';

// maps shoot titles to their portfolio pages
const portfolioRoutes: Record<string, string> = {
  'Corporate Event Shoot': '/corporate-shoot-portfolio',
  'Wedding Shoot': '/wedding-portfolio',
  'Insta & YouTube Video Shoot': '/social-video-portfolio',
  'Commercial Shoot': '/commercial-portfolio',
  'Marketing Shoot': '/marketing-portfolio',
  'Religious Shoot': '/religious-portfolio',
  'Political Shoot': '/political-portfolio',
  'Cinematic Shoot': '/cinematic-portfolio',
};
...
const shootServices = [
  ...
  },
  {
    ...
    features: ['Instagram Reels shoots', 'YouTube video production', 'B-roll footage', 'Multi-camera setup', 'Green screen sessions', 'Professional lighting'],
...
export function ProfessionalShootPage() {
  ...
  const handleShootClick = (title: string) => {
    setSelectedService(title);
    setIsModalOpen(true);
  };

  const handleViewSamples = (title: string) => {
    const route = portfolioRoutes[title];
    if (route) {
      navigate(route);
    } else {
      // fallback to modal for any shoot types without a dedicated page
      setSelectedShootType(title);
  ...
  return (
    <div className="min-h-screen bg-white">
            ...
            {shootServices.map((shoot, index) => (
              ...
              >
                ...
                <div className="p-8">
                  ...
                  <p className="text-slate-500 text-lg font-light mb-6 line-clamp-2">{shoot.description}</p>
      ...
      <ViewSamplesModal isOpen={isSamplesModalOpen} onOpenChange={setIsSamplesModalOpen} shootType={selectedShootType} samples={[]} />
    </div>
  );
}
